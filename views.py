            ...
            if moves >= 5:
                if not choosen:
                    if occupied and tok_type:
                        ...
                        board_matrix, disk_list = touch.get_array()
                        response = JsonResponse({'free': True, 'limits': json.dumps(limits), 'ext_lim': json.dumps(ext_lim), 'previous': previous, 'choosen': choosen, 'occupied': occupied, 'allowed': check_allowed, 'disks': disk_list})
                    else:
