...
class Token(models.Model):
    ...
    def get_array(self):
        board_matrix = [[0 for x in range(-5,6)] for x in range(-5,6)]
        tokens = list(Token.objects.filter(game=self.game_id).values('xn','yn','token_type', 'player'))
        disk_list = [i for i in tokens if int(i['token_type']) == 2]
        disk_list = list(disk_list)
        for i in tokens:
            x = int(i['xn'])
            y = int(i['yn'])
            player = int(i['player'])
            token_type = int(i['token_type'])
            if player == 1:
                token = 10
            else:
                token = 0
            if token_type == 1:
                token += 1
            board_matrix[x][y] = token
        return board_matrix, disk_list
